...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr)) {
    ...
    const bigElement = arr.find(element => element > num);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (Array.isArray(arr)) {
    const slicedArr = arr.slice(startIdx);
    return slicedArr.join(" ");   // для пустого separator треба написати пустий рядок " "!!!
  } else {
    return "Вхідний параметр має бути масивом";
  }
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr)) {
    const newArray = Array.from(arr.values());
    const filteredArray = newArray.filter(elem => elem > 0);
...
function sumAndPrint(arr) {
  if (Array.isArray(arr)) {
    ...
    const sum = arr.reduce((previousValue, currentNumber) => previousValue + currentNumber);
    return sum;
...
function flattenSquareAndFilter(arr, threshold) {
  if (Array.isArray(arr)) {
    const filteredSquaredArray = arr.flat().map((elem) => elem ** 2).filter(elem => elem > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    ...
    const sum = slicedArr.reduceRight((accumulator, initialValue) => accumulator + initialValue);
    slicedArr.forEach((elem) => console.log(elem));
    return sum;
  } else {
    return 'Вхідний параметр має бути масивом';
  }
...
function expandArrayByN(arr, n) {
  ...
  const Fn = function(elem) {
    ...
    for (let i = 1; i <= n; i++) {
      elemArray = [...elemArray, elem];
    }

    return elemArray;
  }
  const expandedArray = arr.flatMap(Fn);
...
function findLongestWord(arr) {
  if (Array.isArray(arr)) {
    ...
    const Fn = function(wordAccumulator, nextWord) {
      if (nextWord.length > wordAccumulator.length) {
        wordAccumulator = nextWord;
      } 
      return wordAccumulator;
    };

    const longestWord = arr.reduce(Fn);
    return longestWord;
...
function findDuplicateElements(arr) {
  if (Array.isArray(arr)) {
      const Fn = (elem, index) => {
        if (index !== arr.indexOf(elem)) {
          return elem;
        }
      }
      const duplicateElement = arr.filter(Fn);
...
function capitalizeWords(arr) {
  if (Array.isArray) {
    ...
    const CapitalizeFn = (elem) => {
      const arrFromWord = Array.from(elem); //перетворили рядок на масив
      const capFirstLetter = arrFromWord.shift().toUpperCase();//видалили першу букву і зробили її великою 
      ///////////////////////////////////////////////////////arrFromWord =[ 'p', 'p', 'l', 'e' ]
      arrFromWord.unshift(capFirstLetter); //.unshift() повертаэ довжини слів: 5,6,6; а масив модифікується!
      // arrFromWord повертає = [ 'A', 'p', 'p', 'l', 'e' ]
      //[ 'B', 'a', 'n', 'a', 'n', 'a' ]
      //[ 'O', 'r', 'a', 'n', 'g', 'e' ]
    ...
    const capitalizedArray = arr.map(CapitalizeFn);
...
function calculateTotalPrice(arr) {
  if (Array.isArray(arr)) {
    ...
    const initPrice = 0;
    const totalPrice = arr.reduce((sum, currentItem) => {
